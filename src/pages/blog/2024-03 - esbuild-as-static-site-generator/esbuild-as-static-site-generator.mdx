...
simplified by using Github pages and 11ty.

The last part of the post explains about MDX, the obvious combination of
Markdown and JSX. This post continues from there and looks at how 11ty can be
replaced by the newest Javascript bundler Esbuild.
...
Setting up `esbuild` and `@mdx-js/esbuild is` straightforward and is [clearly
explained](https://mdxjs.com/packages/esbuild/) on mdxjs.com.

This setup makes Esbuild transform MDX to Javascript bundles. Additional
functionality can be added by using plugins from the rich MDX ecosystem. The
example below shows how to add support for: 
...
All that is left is transforming the generated Javascript bundles to HTML pages.
More on that in the next post.

<Topic
  data-variant="card"
  globals={globals}
  topic={getOpenerTopicByNavigationId({ pages: props.pages, navigationId: 'jsx-render-plugin' })}
  images={blogImages}
  swap-image={true}
/>
