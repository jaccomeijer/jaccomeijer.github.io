...
### Use wit JSX based frameworks

The [green-build](https://github.com/jaccomeijer/green-build) package is an
excremental Proof Of Concept. It is used to build this this site and it was used
to create the UI library.

Using the UI library with a JSX based framework requires some work. A quick test
with [Astro](https://astro.build) learns that the `*.jsx` files exported from
the library must be bundled first.

Once bundled the dev server shows warnings because of React conventions like
`key` attributes and `class` attributes that should be named into `className`.
Nevertheless, the component renders properly.

### Future

A future version of the library might become fully React compatible. That way,
the library could be used for simple static websites, as well as for more
