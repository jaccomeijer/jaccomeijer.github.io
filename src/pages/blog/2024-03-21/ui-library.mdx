...
isolation makes sharing styles harder. That's why [CSS Shadow
Parts](https://www.w3.org/TR/css-shadow-parts-1/) exist.

### No build required

The library does not have a build step. To publish the documentation, the MDX
documents have to be rendered. Publishing to the package repository simply
copies all `.jsx` and `.css`.

### Use with Astro

[Astro](https://astro.build) components are **HTML-only components with no
client-side runtime**. This library was designed to provide exactly that but
requires the framework to transform the `.jsx` into HTML.

For Astro this can be achieved by using the components in `MDX` documents.

### Use with React

Using the library with React requires some work. A quick test learns that the
`.jsx` files exported from the library must be bundled first.

Once bundled using the components shows warnings because of React conventions
like missing `key` attributes and `class` attributes that should be named
`className`. Nevertheless, the component renders properly.
