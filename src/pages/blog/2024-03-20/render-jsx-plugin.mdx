...
The plugin above uses Preact to transform JSX into static HTML. Because the
Factory functions are added when bundling and removed again when rendering.
Theoretically, the JSX framework should not affect the final result. In
practice, there's some differences.
...
contains two **deprecated** functions:
...
Of which the Static variants are the most useful here.
...
This not not mean that client side Javascript cannot be used with a static
...
the two and creating a reusable and documented UI library is simpler than ever.
The next post zooms in on this.
