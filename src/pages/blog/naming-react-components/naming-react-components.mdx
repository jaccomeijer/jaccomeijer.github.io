...
`<Heading>This is the hading</Heading> <Abstract>This is the abstract</Abstract>
...
`<Topic model="{topic}" /> <Navigation model="{navigation}" /> <Action
model="{action}" /> <Media model="{media}" />`
...
`<OneColumnLayout> <SomeModel /> <SomeModel /> </SidebarLayout>
<TwoColumnLayout> <Topic /> <Topic /> </TwoColumnLayout>`
